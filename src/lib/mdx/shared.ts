export const htmlAttributeToReactPropMap: Record<string, string> = {
  // --- HTML special cases ---
  class: 'className',
  for: 'htmlFor',
...
export function htmlAttributeToReactProp(attribute: string) {
  if (attribute.startsWith('data-') || attribute.startsWith('aria-')) {
    return attribute;
  }
  const lowered = attribute.toLowerCase();
  if (Object.hasOwn(htmlAttributeToReactPropMap, lowered)) {
    return htmlAttributeToReactPropMap[lowered];
  }
  return attribute;
}
